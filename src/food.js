...
export default class Food extends React.Component
{
    constructor(props)
    {
        ...
        this.state = {
            ...
            distanceLeft: "0%",
            dayMinus: 0,
            currentGraphFocus: "Calories"
        }
        if(SyncStorage.get('todaysKcal' + month + year + day) == undefined)
        {
            this.state.todaysKcal = 0;
        }
        if(SyncStorage.get('todaysFat' + month + year + day) == undefined)
        {
            this.state.todaysFat = 0;
        }
        if(SyncStorage.get('todaysCarbs' + month + year + day) == undefined)
        {
            this.state.todaysCarbs = 0;
        }
        if(SyncStorage.get('todaysProtein' + month + year + day) == undefined)
        {
            this.state.todaysProtein = 0;
        }
        if(SyncStorage.get('breakfastKcal' + month + year + day) == undefined)
        {
            this.state.breakfastKcal = 0;
        }
        if(SyncStorage.get('breakfastFat' + month + year + day) == undefined)
        {
            this.state.breakfastFat = 0;
        }
        if(SyncStorage.get('breakfastCarbs' + month + year + day) == undefined)
        {
            this.state.breakfastCarbs = 0;
        }
        if(SyncStorage.get('breakfastProtein' + month + year + day) == undefined)
        {
            this.state.breakfastProtein = 0;
        }
        if(SyncStorage.get('lunchKcal' + month + year + day) == undefined)
        {
            this.state.lunchKcal = 0;
        }
        if(SyncStorage.get('lunchFat' + month + year + day) == undefined)
        {
            this.state.lunchFat = 0;
        }
        if(SyncStorage.get('lunchCarbs' + month + year + day) == undefined)
        {
            this.state.lunchCarbs = 0;
        }
        if(SyncStorage.get('lunchProtein' + month + year + day) == undefined)
        {
            this.state.lunchProtein = 0;
        }
        if(SyncStorage.get('dinnerKcal' + month + year + day) == undefined)
        {
            this.state.dinnerKcal = 0;
        }
        if(SyncStorage.get('dinnerFat' + month + year + day) == undefined)
        {
            this.state.dinnerFat = 0;
        }
        if(SyncStorage.get('dinnerCarbs' + month + year + day) == undefined)
        {
            this.state.dinnerCarbs = 0;
        }
        if(SyncStorage.get('dinnerProtein' + month + year + day) == undefined)
        {
            this.state.dinnerProtein = 0;
        }
        if(SyncStorage.get('snacksKcal' + month + year + day) == undefined)
        {
            this.state.snacksKcal = 0;
        }
        if(SyncStorage.get('snacksFat' + month + year + day) == undefined)
        {
            this.state.snacksFat = 0;
        }
        if(SyncStorage.get('snacksCarbs' + month + year + day) == undefined)
        {
            this.state.snacksCarbs = 0;
        }
        if(SyncStorage.get('snacksProtein' + month + year + day) == undefined)
        {
    ...
    getGraphData = () =>
    {
        ...
        var tmparray = [];
        var name = this.state.currentGraphFocus;
        for(var i = 0; i <= 23; i += 2)
        {
            var tmp = SyncStorage.get(name + month + year + day + i);
            if(tmp == undefined)
            {
                tmp = 0;
            }
    ...
    changegraph(id)
    {
        ...
        if(id == "Carbohydrates")
        {
            this.setState(
            {
                currentGraphFocus: "Carbohydrates"
            });
        }
    }
    getLeft(i)
    ...
    addToFocused = (addOrSub, id) =>
    {
        ...
        if(id == "breakfast")
        {
            if(this.state.currentFocus == "Calories")
            {
                ...
                if(addOrSub == "-" && this.state.breakfastKcal >= calAdder)
                {
                    this.setState(
                    {
                        breakfastKcal: this.state.breakfastKcal - calAdder,
                        todaysKcal: this.state.todaysKcal - calAdder
                    });
                }
                if(addOrSub == "+")
                {
            ...
            if(this.state.currentFocus == "Protein")
            {
                if(addOrSub == "-" && this.state.breakfastProtein >= protineadder)
                {
                    this.setState(
                    {
                        breakfastProtein: this.state.breakfastProtein - protineadder,
                        todaysProtein: this.state.todaysProtein - protineadder
                    });
                }
                if(addOrSub == "+")
                {
            ...
            if(this.state.currentFocus == "Carbohydrates")
            {
                if(addOrSub == "-" && this.state.breakfastCarbs >= carbAdder)
                {
                    this.setState(
                    {
                        breakfastCarbs: this.state.breakfastCarbs - carbAdder,
                        todaysProtein: this.state.todaysProtein - carbAdder
                    });
                }
                if(addOrSub == "+")
                {
                    ...
                    {
                        breakfastCarbs: this.state.breakfastCarbs + carbAdder,
                        todaysCarbs: this.state.todaysCarbs + carbAdder
                    });
                }
            }
            if(this.state.currentFocus == "Fat")
            {
                if(addOrSub == "-" && this.state.breakfastFat >= fatAdder)
                {
                    this.setState(
                    {
                        breakfastFat: this.state.breakfastFat - fatAdder,
                        todaysFat: this.state.todaysFat - fatAdder
                    });
                }
                if(addOrSub == "+")
                {
        ...
        if(id == "lunch")
        {
            if(this.state.currentFocus == "Calories")
            {
                if(addOrSub == "-" && this.state.lunchKcal >= calAdder)
                {
                    this.setState(
                    {
                        lunchKcal: this.state.lunchKcal - calAdder,
                        todaysKcal: this.state.todaysKcal - calAdder
                    });
                }
                if(addOrSub == "+")
            ...
            if(this.state.currentFocus == "Protein")
            {
                if(addOrSub == "-" && this.state.lunchProtein >= protineadder)
                {
                    this.setState(
                    {
                        lunchProtein: this.state.lunchProtein - protineadder,
                        todaysProtein: this.state.todaysProtein - protineadder
                    });
                }
                if(addOrSub == "+")
            ...
            if(this.state.currentFocus == "Carbohydrates")
            {
                if(addOrSub == "-" && this.state.lunchCarbs >= carbAdder)
                {
                    this.setState(
                    {
                        lunchCarbs: this.state.lunchCarbs - carbAdder,
                        todaysProtein: this.state.todaysProtein - carbAdder
                    });
                }
                if(addOrSub == "+")
            ...
            if(this.state.currentFocus == "Fat")
            {
                if(addOrSub == "-" && this.state.lunchFat >= fatAdder)
                {
                    this.setState(
                    {
                        lunchFat: this.state.lunchFat - fatAdder,
                        todaysFat: this.state.todaysFat - fatAdder
                    });
                }
                if(addOrSub == "+")
        ...
        if(id == "dinner")
        {
            if(this.state.currentFocus == "Calories")
            {
                if(addOrSub == "-" && this.state.dinnerKcal >= calAdder)
                {
                    this.setState(
                    {
                        dinnerKcal: this.state.dinnerKcal - calAdder,
                        todaysKcal: this.state.todaysKcal - calAdder
                    });
                }
                if(addOrSub == "+")
            ...
            if(this.state.currentFocus == "Protein")
            {
                if(addOrSub == "-" && this.state.dinnerProtein >= protineadder)
                {
                    this.setState(
                    {
                        dinnerProtein: this.state.dinnerProtein - protineadder,
                        todaysProtein: this.state.todaysProtein - protineadder
                    });
                }
                if(addOrSub == "+")
            ...
            if(this.state.currentFocus == "Carbohydrates")
            {
                if(addOrSub == "-" && this.state.dinnerCarbs >= carbAdder)
                {
                    this.setState(
                    {
                        dinnerCarbs: this.state.dinnerCarbs - carbAdder,
                        todaysProtein: this.state.todaysProtein - carbAdder
                    });
                }
                if(addOrSub == "+")
            ...
            if(this.state.currentFocus == "Fat")
            {
                if(addOrSub == "-" && this.state.dinnerFat >= fatAdder)
                {
                    this.setState(
                    {
                        dinnerFat: this.state.dinnerFat - fatAdder,
                        todaysFat: this.state.todaysFat - fatAdder
                    });
                }
                if(addOrSub == "+")
        ...
        if(id == "snacks")
        {
            if(this.state.currentFocus == "Calories")
            {
                if(addOrSub == "-" && this.state.snacksKcal >= calAdder)
                {
                    this.setState(
                    {
                        snacksKcal: this.state.snacksKcal - calAdder,
                        todaysKcal: this.state.todaysKcal - calAdder
                    });
                }
                if(addOrSub == "+")
            ...
            if(this.state.currentFocus == "Protein")
            {
                if(addOrSub == "-" && this.state.snacksProtein >= protineadder)
                {
                    this.setState(
                    {
                        snacksProtein: this.state.snacksProtein - protineadder,
                        todaysProtein: this.state.todaysProtein - protineadder
                    });
                }
                if(addOrSub == "+")
            ...
            if(this.state.currentFocus == "Carbohydrates")
            {
                if(addOrSub == "-" && this.state.snacksCarbs >= carbAdder)
                {
                    this.setState(
                    {
                        snacksCarbs: this.state.snacksCarbs - carbAdder,
                        todaysProtein: this.state.todaysProtein - carbAdder
                    });
                }
                if(addOrSub == "+")
            ...
            if(this.state.currentFocus == "Fat")
            {
                if(addOrSub == "-" && this.state.snacksFat >= fatAdder)
                {
                    this.setState(
                    {
                        snacksFat: this.state.snacksFat - fatAdder,
                        todaysFat: this.state.todaysFat - fatAdder
                    });
                }
                if(addOrSub == "+")
    ...
    getGraphColor()
    {
        console.log(this.state.currentGraphFocus);
        if(this.state.currentGraphFocus == "Protein")
        {
            return "rgb(0,156,255)";
        }
        else if(this.state.currentGraphFocus == "Carbohydrates")
        {
            return "rgb(14,209,69)";
        }
        else if(this.state.currentGraphFocus == "Fat")
        {
            return "rgb(210,185,60)";
        }
        else
        {
    ...
    render()
    {
        ...
        return (
          ...
          <View style={{ position:"absolute",top: height* 0.285  ,justifyContent: "center",alignItems:"center",flexDirection:"row", left:this.getLeft(3) ,   height: height*0.08    ,width: "100%",  borderRadius:15  }}>


          <TouchableOpacity style={{justifyContent: "center",alignItems:"center",  backgroundColor:"white", height: "90%"   ,width:  "20%",  borderRadius:15, margin:4,borderWidth:2 , borderColor:"rgb(240,126,126)" }}  onPress={() => {return this.changegraph("Calories")  }} >

            <Text  style={{fontSize: height*0.018 ,color:"black" ,marginLeft:3    }}>Calories</Text>
          </TouchableOpacity>

          <TouchableOpacity style={{justifyContent: "center",alignItems:"center",  backgroundColor:"white", height: "90%"   ,width:  "20%",  borderRadius:15, margin:4,borderWidth:2 , borderColor:"rgb(255,211,77)" }}  onPress={() => {return this.changegraph("Fat")  }} >

            <Text  style={{fontSize: height*0.018 ,color:"black" ,marginLeft:3    }}>Fat</Text>
          </TouchableOpacity>
          <TouchableOpacity style={{justifyContent: "center",alignItems:"center",  backgroundColor:"white", height: "90%"   ,width:  "20%",  borderRadius:15, margin:4,borderWidth:2 , borderColor:"rgb(0,156,255)" }}  onPress={() => {return this.changegraph("Protein")  }} >

            <Text  style={{fontSize: height*0.018 ,color:"black" ,marginLeft:3    }}>Protein</Text>
          </TouchableOpacity>
          <TouchableOpacity style={{justifyContent: "center",alignItems:"center",  backgroundColor:"white", height: "90%"   ,width:  "20%",  borderRadius:15, margin:4,borderWidth:2 , borderColor:"rgb(14,209,69)" }}  onPress={() => {return this.changegraph("Carbohydrates")  }} >

            <Text  style={{fontSize: height*0.018 ,color:"black" ,marginLeft:3    }}>Carb's</Text>
          </TouchableOpacity>
                   ...
                   chartConfig={{
                     ...
                     decimalPlaces: 0,
                     color: (opacity = 1) => this.getGraphColor(),
                  ...
                  <Text style={{color:"darkorange",position:"absolute",top:"88%",      left:this.getLeft(3), fontSize: 18, fontWeight: "50"  ,  textAlign: 'center', marginTop: 3,   marginLeft:"3%",  height: 96}}>Keeping a steady healthy diet is a vital component of keeping PCOS under control.</Text>
