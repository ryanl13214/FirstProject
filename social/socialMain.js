/** @format */
import React from 'react';
import
{
    Animated,
    Image,
    PixelRatio,
    FlatList,
    StyleSheet,
    Text,
    TouchableOpacity,
    View,
    ScrollView,
    Button,
    SafeAreaView,
    Easing
}
from 'react-native';
import
{
    Dimensions
}
from 'react-native';
import
{
    WebView
}
from 'react-native-webview';
const
{
    width,
    height
} = Dimensions.get('window');
import * as ip from '../ipAddress.json';
const ipaddress = ip.ip;
import SyncStorage from 'sync-storage';
const Item = (
{
    item,
    conents,
    user,
    heightTextbox,
    widthTextbox,
}) => (
    <View style={{
flexDirection: 'row'  ,
borderColor: '#1fffff',
borderBottomWidth:1,
marginTop: 7,
marginLeft: 15,
   minHeight: 70,
   height:heightTextbox/3  ,
 justifyContent:  "flex-start" ,
   width:width*0.7
 }}>

   <View style={{flexDirection: 'row'  ,   height: heightTextbox   ,justifyContent:  "flex-start"  }}>

     <Image style={{borderColor: '#ffffff',  borderWidth:1, width:30, height:30 , borderRadius:27  }}  source={require('../imgs/placeholdericonhomepage.png')} />

         <View style={{


           marginLeft: 5,
           marginRight: 5,
           minHeight: 30,
           backgroundColor:"white",
           height: heightTextbox  ,

           width:widthTextbox
         }}>
         <Text style={{  marginLeft:15,  fontSize: 16,color:"black"     }}>{conents}</Text>
         </View>



   </View>

 </View>
);
const LeftChatItem = (
{
    id,
    type,
    name
}) => (
    <View style={{

 }}>

 <TouchableOpacity style={{    width:width*0.17, height:  width*0.17 ,marginLeft: (width*0.16)*0.5,marginTop: (width*0.063)*0.5 }}   >
<Image style={{ width: "100%", height:  "100%",    resizeMode:"stretch",borderRadius:60}} source={require('../imgs/placeholdericonhomepage.png')} />
 </TouchableOpacity>
 <Text style={{  width:width*0.32 ,fontSize: 15,color:"black"   ,    textAlign: 'center'  }}> {name}</Text>



 </View>
);
export default class SocialMain extends React.Component
    {
        constructor(props)
        {
//  this.callFunc = this.callFunc.bind(this);
          var tmpArraymsg = [];
            super(props);
            var tmpArray = [];
            var userId = SyncStorage.get('currentUID');
            let body = JSON.stringify(
            {
                userid: userId.toString()
            })
            var fulladdress = ipaddress + "/listOfUserChats"
            fetch(fulladdress,
                {
                    method: 'POST',
                    headers:
                    {
                        'Accept': 'application/json',
                        'Content-Type': 'application/json',
                        'Content-Length': body.length
                    },
                    body: body,
                }).then((response) => response.json())
                .then((responseData) =>
                {
                    ////
                //    console.log("---" + JSON.stringify(responseData));
                    //console.log("-" + responseData.rows.length);
                    for(var i = 0; i < responseData.rows.length; i++)
                    {
                        tmpArray.push(
                        {
                            id: responseData.rows[i][0],
                            type: responseData.rows[i][1],
                            name: responseData.rows[i][2]
                        });
                      //  console.log("---" + JSON.stringify(tmpArray));
                    }
                    /////






                    if(tmpArray.length != 0 ){
                        console.log("TEMP DATA 1 DUMP");
                      ///////////////////////////// getting the chat history

                      var fulladdress = ipaddress + "/getmessages"
                      let body = JSON.stringify(
                      {
                          chatID:"1"// chatIdhere
                      })
                      fetch(fulladdress,
                          {
                              method: 'POST',
                              headers:
                              {
                                  'Accept': 'application/json',
                                  'Content-Type': 'application/json',
                                  'Content-Length': body.length
                              },
                              body: body,
                          }).then((response) => response.json())
                          .then((responseData) =>
                          {
                              ////
                              console.log("DATA DUMP--" + JSON.stringify(responseData));
                              console.log("LENGHT-" + responseData.rows.length);
                              for(var i = 0; i < responseData.rows.length; i++)
                              {

                                  tmpArraymsg.push(
                                  {
                                      id: responseData.rows[i][0],
                                      conents: responseData.rows[i][2],
                                      user: "system"
                                  });
                                  console.log("-U--" + JSON.stringify(tmpArraymsg));
                              }
                              /////
                          })
                          .catch((err) =>
                          {
                              console.log(err);
                          });









                    }












                    ///////
                })
                .catch((err) =>
                {
                    console.log(err);
                });
          //  console.log("statetmpArray");












            console.log(tmpArray);

            this.state = {
                inputText: "",
                listOfChats: tmpArray,
                messages: tmpArraymsg,
                base64userImage: ""
            };
        }
        getchatheight = () =>
        {
            if(this.state.inputText.length == 0)
            {
                return "72%";
            }
            return "68%";
        }


        callFunc(a,b)
        {
            this.setState(
            {
                isVisible: !this.state.isVisible
            });
        }

        render()
            {
                const renderItem = (
                {
                    item,
                    conents,
                    user
                }) => (
                    <Item   conents={item.conents} user={item.user}   heightTextbox ={getheight(item)} widthTextbox={getwidth(item)}              />
                );
                const renderLeftChatItem = (
                {
                    item,
                    id,
                    type,
                    name
                }) => (
                    <LeftChatItem   id={item.id} type={ item.type} name={ item.name}        />
                );
                var getheight = (a) =>
                {
                    var textSpacing = (((width - 30) * 0.7) / 16) * 2;
                    if(a.videowidth == 0)
                    {
                        return ((a.conents.length / textSpacing) * 27) + 40;
                    }
                    if(a.videowidth == 2)
                    {
                        return height * 0.3;
                    }
                    if(a.conents.length == 0)
                    {
                        return 0;
                    }
                    return 200;
                }
                var getwidth = (a) =>
                {
                    var textSpacing = ((width - 30) * 0.7) / 20
                    if(a.videowidth == 0)
                    {
                        if(a.conents.length == 0)
                        {
                            return 0;
                        }
                        else if(a.conents.length > textSpacing * 2)
                        {
                            return "71%";
                        }
                        else
                        {
                            return Math.floor(textSpacing * ((width - 30) * 0.7) / 18);
                        }
                    }
                    else
                    {
                        return 200;
                    }
                }
                return (

			<SafeAreaView style={styles.container}>
			<Image style={{position:"absolute",justifyContent: "center",alignItems:"center",  width: width, height: height,resizeMode: 'stretch',top:0 }} source={require('../imgs/revision3/socialMain1.jpg')} />





              <View style={{position:"absolute", top:height*0.176 ,left:"0%"  ,  width:width*0.32, height:height*0.57  }}>




              <FlatList
              numColumns={1}
              horizontal={false}
                data={this.state.listOfChats}
                renderItem={renderLeftChatItem}
                keyExtractor={leftChatItem => leftChatItem.id}
                style={{  }}
              />

              </View>



              < View style={{width:width*0.6,height:this.getchatheight(),marginTop:"34%",marginLeft: width*0.34  }} >
                  <FlatList
                  numColumns={1}
                  horizontal={false}
                    data={	this.state.messages}
                    renderItem={renderItem}
                    keyExtractor={item => item.id}
                    style={{  }}
                  />
            </ View>







      			   <TouchableOpacity style={{position:"absolute"  ,top:(height*0.84)-70   ,left:0 ,     width:width*0.27, height:  height*0.16   }}  onPress={() =>  this.props.navigation.navigate('GroupFinder')} >
      			<Image style={{ width: "100%", height:  "100%",    resizeMode:"stretch",borderTopRightRadius:60}} source={require('../imgs/revision3/findgroups.jpg')} />

      			   </TouchableOpacity>











			</SafeAreaView>




		);
	}
}

const styles = StyleSheet.create({







});
